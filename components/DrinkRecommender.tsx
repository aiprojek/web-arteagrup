...
const DrinkRecommender: React.FC = () => {
    ...
    const getRecommendation = async () => {
        ...
        const menu = getMenuForOutlet('semua');
    ...
    const renderLocalRecommendationText = () => {
        ...
        switch (localResult.reason) {
            case 'Rekomendasi Hari Ini':
                return `Tentu! **Rekomendasi Hari Ini** dari Asisten AI jatuh kepada... **${localResult.drink}**! Minuman ini pas banget buat nemenin harimu. Selamat mencoba!`;
    ...
    return (
        <div className="flex flex-col h-full">
            <div className="text-center">
                 <h2 className="text-2xl font-bold text-white mb-2">Asisten AI Artea Grup</h2>
                 <p className="text-stone-400 mb-6">
                    Punya pertanyaan? Tanya apa saja seputar menu, lokasi, atau tentang kami.
                 </p>
            </div>
                ...
                {(localResult || geminiResult) && (
                    <div className="mt-6 bg-stone-900/50 p-4 rounded-lg border border-stone-700 animate-fade-in-content">
                        <h3 className="font-semibold text-white mb-2 flex items-center">
                            <i className="bi bi-chat-quote-fill mr-2 text-[var(--accent-color)]"></i>
                            {localResult?.type === 'definition' ? 'Info Cepat dari Asisten AI:' : 'Jawaban dari Asisten AI:'}
                        </h3>
